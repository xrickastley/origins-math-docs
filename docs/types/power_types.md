# Power Types

Power Types are what grants functionality to your origins! Each power has a type, specified with a `type` field in the JSON. Which type a power is defines which other fields it requires and supports.

### Additional Resource Types
- [Attribute-like Resource](./power_types/attribute_like_resource.md)
- [Math Resource](./power_types/math_resource.md)
- [Modifiable Resource](./power_types/modifiable_resource.md)

### Linked Resource types
- [Attribute Linked Resource](./power_types/attribute_linked_resource.md)
- [Current Biome Linked Resource](./power_types/current_biome_linked_resource.md)
- [Damage Dealt Linked Resource](./power_types/damage_dealt_linked_resource.md)
- [Damage Taken Linked Resource](./power_types/damage_taken_linked_resource.md)
- [Living Entity Linked Resource](./power_types/living_entity_linked_resource.md)
- [Healing Linked Resource](./power_types/healing_linked_resource.md)
- [Nbt-linked Resource](./power_types/nbt_linked_resource.md)
- [Player Linked Resource](./power_types/player_linked_resource.md)
- [Scoreboard Linked Resource](./power_types/scoreboard_linked_resource.md)
- [Status Effect Linked Resource](./power_types/status_effect_linked_resource.md)

### Modifying types
- [Modify Attribute-like Resource](./power_types/modify_attribute_like_resource.md)
- [Modify Resource Maximum](./power_types/modify_resource_maximum.md)
- [Modify Resource Minimum](./power_types/modify_resource_minimum.md)
- [Modify Simple](./power_types/modify_simple.md)